-- 019_otp_pgcrypto_search_path.sql
-- The 018 functions SET search_path = public, but pgcrypto lives in the
-- `extensions` schema on cloud (and modern local CLI), so gen_salt()/crypt()
-- could not resolve. Recreate with search_path = public, extensions.
-- (Local environments that installed pgcrypto into public still resolve.)
