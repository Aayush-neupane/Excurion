-- 020_otp_internal_role_grant.sql
-- otp_register promotes the just-created profile to the role the user
-- chose, but the profiles role-change guard (see 010/012) blocks it because
-- the authenticated session carries auth.uid(). Give server functions an
-- explicit, transaction-scoped bypass flag instead of relying on session
-- conventions.
...
as $$
begin
  if coalesce(current_setting('excurion.internal_role_grant', true), 'off') = 'off'
